...
def main(grid: Grid, context: Context) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"DELAYED FREE RIDER ATTACK EXPERIMENT")
    print(f"{'='*60}")
    print(f"Dataset: {dataset.upper()}")
    print(f"Partitioning: {partitioning.upper()}")
    if partitioning.lower() == "noniid":
        print(f"Dirichlet Alpha: {dirichlet_alpha}")
    print(f"Attack: Delayed Free Riders")
    print(f"  - Phase 1 (Rounds 1-5): Train normally, store weights")
    print(f"  - Phase 2 (Rounds 6+): Send running average of last 5 weights")
    print(f"  - 30% malicious clients (0-29)")
    ...
    if aggregator.lower() == "fedmedian":
        ...
        print("Using MEDIAN aggregation (robust to delayed free riders)")
        ...
        print("Using FEDAVG aggregation (vulnerable to delayed free riders)")
    print(f"{'='*60}\n")
...
def create_global_evaluate(aggregator: str, dataset: str, partitioning: str):
    """Create evaluation function with delayed attack-specific CSV logging."""
    ...
    def global_evaluate(server_round: int, arrays: ArrayRecord) -> MetricRecord:
        ...
        # CSV filename includes dataset, partitioning, and aggregator - saved in results/
        csv_filename = f"results/delayed_freerider_{dataset}_{partitioning}_{aggregator}.csv"
